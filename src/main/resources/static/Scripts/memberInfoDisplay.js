...
$(document).click(function(event) { 
	$target = $(event.target);
	if(!$target.closest('.mem-opt-popup').length && !$target.closest('.matrilinealSelect').length && !$target.closest('.name-input-popup').length && !$target.closest('.marryOptionsPopup').length && !$target.closest('.name-input-popup').length && popupDisplay) {
